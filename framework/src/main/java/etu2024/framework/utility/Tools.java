...
public class Tools {
  public static Method getMethodByName(Class<?> type, String name) {
    ...
    // For each method, check if the name is the same as the one given in
    // parameter
  ...
  public static Object cast(Class<?> type, Object value) throws IOException {
    if (type.isArray()) {
      ...
      // For each element in the value, cast it to the array type and add it to
      // the array
      ...
      try {
        ...
        Class<?> wrapperClass = Class.forName(
            "java.lang." + type.getName().substring(0, 1).toUpperCase() +
            type.getName().substring(1));
        // Get the parse method
        Method method = wrapperClass.getMethod(
            "parse" + type.getName().substring(0, 1).toUpperCase() +
                type.getName().substring(1),
            String.class);
        return method.invoke(null, String.valueOf(value));
      } catch (NoSuchMethodException | IllegalAccessException |
               InvocationTargetException | ClassNotFoundException e) {
        throw new MethodNotFoundException("FRAMEWORK ERROR - The type " +
      ...
      try {
        ...
        try {
          ...
          if (String.valueOf(value).trim().isEmpty())
            return null;
          try {
            // Try to get the parse method with a String parameter (mostly for
            ...
            throw new MethodNotFoundException(
                "FRAMEWORK ERROR - The framework does not support " +
                type.getName() + " yet!");
          }
