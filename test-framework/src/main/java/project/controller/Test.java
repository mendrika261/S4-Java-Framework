...
public class Test {
  ...
  // The @auth annotation can take a list of profiles as arguments
  ...
  public ModelView json() {
    ...
    modelView.addItem("someAttribute",
                      "This is a JSON Response from data set in modelView");
  ...
  @Url(url = "/file")
  @Auth
  public ModelView file(File file, String[] tag, LocalDateTime localDateTime,
                        LocalDate localDate, LocalTime localTime)
      throws IOException {
    // Like all other parameters (and attributes), the file is automatically get
    // from the request
    ModelView modelView = new ModelView();

    if (file != null) { // To check if it is the post request
      modelView.addItem("file", file.getName() + ": " +
                                    file.getContent().length +
                                    " bytes received");
      file.uploadToFile(
          "/Users/mendrika/IdeaProjects/S4-Java-Framework/test-framework/output/");
    }
